...
現在のグローバル skill は `.apm/apm.yml` で管理する。APM 0.14.2 の user scope では `targets:` ではなく `target: claude,agent-skills` を使う。`apm.lock.yaml` と `apm_modules/` は `.apm/.gitignore` で除外する。`retrospective-codify` は `github:mizchi/skills`、`autoreview` は `github:openclaw/agent-skills` を参照する。`gogcli` は未使用のため廃止済み。

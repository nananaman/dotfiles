...
test_codex_wrapper_uses_the_packaged_entrypoint() {
  ...
  # Arrange: Codexのpackageは公開entrypointをbin/codexとして提供する。
  # Act: sandbox wrapperが起動するpackage内commandをsourceから確認する。
  # Assert: 廃止されたcodex-rawではなく、packageの公開entrypointへ委譲する。
  rg -q -F '${codexCliPackage}/bin/codex --sandbox' "$packages_module"
